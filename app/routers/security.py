...
@router.get("/{id}",response_model=schema.SecurityGroup)
async def read_security_group(id: int, db=Depends(get_db)):
    db_sg = crud_security.get_security_group_by_id(db, id=id)
    if db_sg is None:
